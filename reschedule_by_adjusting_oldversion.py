...
def select_candidates_by_room_time(scheduled_df, allowed_room_codes, preferred_options):
    """
    从已排课表中筛选：教室在 allowed_room_codes 内，且上课时段落在失败课程的可用时间 preferred_options 内的课程。
    返回筛选后的 DataFrame（可再取 unique_jxbids）。
    """
    if scheduled_df is None or scheduled_df.empty or not preferred_options or not allowed_room_codes:
        return scheduled_df.iloc[0:0]

    day_map = {"周一": 0, "周二": 1, "周三": 2, "周四": 3, "周五": 4, "周六": 5, "周日": 6}
    rows = []

    for idx, rec in scheduled_df.iterrows():
        room_code = str(rec.get('教室代码', '')).strip()
        if room_code not in allowed_room_codes:
            continue

        day = day_map.get(str(rec.get('星期', '')).strip(), None)
        if day is None:
            continue

        # 解析节次，转为 0-based 的开始、结束（含）
        period_text = str(rec.get('节次', '')).replace("第", "").replace("节", "")
        if "-" in period_text:
        ...
        for opt in preferred_options:
            days_spec = opt.get('days', 'all')
            days_list = list(range(7)) if days_spec == 'all' else opt.get('days', [])
            if day not in days_list:
                continue

            periods_spec = opt.get('periods', 'all')
            if periods_spec == 'all':
                match_opt = True
                break
            for s, e in periods_spec or []:
...
def re_with_match_courses_betchs_new(
    ...
    try:
        for info in candidates_info:
            ...
            clist_of_classes = info['classes_list']
            #clist_of_classes=get_class_instances1(candidate_jxbs[0], classes,courses)  ##取出正确的班级，还没有放弃检查班级冲突
            ...
            # 候选允许的教室池（宽松）
            # campus = getattr(candidate_jxbs[0], 'SKXQ', None)
            # candidate_capacity = getattr(candidate_jxbs[0], 'KRL', 0)
            # suitable_classrooms = [
            #     room for room in classrooms
            #     if room.SFYXPK == '1' and room.SKZWS >= candidate_capacity and (campus is None or room.MC == campus)
            # ]
            suitable_classrooms=filter_suitable_classrooms(classrooms, candidate_jxbs[0], relax_constraints=True)
            #re_list_of_classes = get_class_instances1(candidate_jxbs[0], classes, courses)  ##取出正确的班级，还没有放弃检查班级冲突
            re_list_of_classes = []
            ...
            for room in suitable_classrooms:
                ...
                for pattern in day_patterns:
                    days_list, hours_list = pattern
                    arrangements = new_check_period_availability(
                        days_list, hours_list, room, cteacher_week_map, re_list_of_classes, teaching_weeks, candidate_time_constraints
                    )
                    alternative_arrangements.extend(arrangements)

                # 选一个与原安排不同的方案,不需要不同。选第一个安排
                for arrangement in alternative_arrangements:
                    is_different = True
                    # for orig_day, orig_period, orig_hours in current_arrangements:
                    #     for new_day, new_period, new_hours in arrangement:
