...
`scripts/timetable_agent_tools.py` 提供诊断、试排与方案比较的 JSON Schema / `dispatch_tool` 接口，供后续 agent 调用，见 [Agent排课工具接口](docs/Agent排课工具接口.md)。2026-09-25 获授权的学院时段日间配置联合补排89门，原课程移动0门，原162门失败清单还剩73门；逐课原因见 [74门复核与日间73门说明](docs/74门复核与当前73门说明.md)，论文方法的接入边界见 [三篇论文接入设计](docs/三篇论文接入设计.md)。

最新含晚间方案联合补排96门，剩66门，原课表移动0门；单节任意起点已参数化。日间与晚间结果对照、剩余原因及实现边界见 [本轮修复结果与工具说明](docs/本轮修复结果与工具说明.md)。
