...
# ===== 图8: 教室利用率 =====
TOTAL_SLOTS = 5 * 3 - 1
room_usage = df.groupby('教室').agg(已排班次=('教学班ID','count'), 校区=('校区','first')).reset_index()
room_usage['利用率'] = (room_usage['已排班次'] / TOTAL_SLOTS * 100).clip(upper=200)
fig, axes = plt.subplots(1, 2, figsize=(14, 5.5))
fig.suptitle('教室利用率分析', fontsize=14, fontweight='bold')
bins = [0,20,40,60,80,100,999]; labels_u = ['0–20%','20–40%','40–60%','60–80%','80–100%','>100%']
ucounts = pd.cut(room_usage['利用率'], bins=bins, labels=labels_u).value_counts().reindex(labels_u).fillna(0)
bcolors = ['#FFCDD2','#FF8A65','#FFA726','#66BB6A','#1B5E20','#B71C1C']
...
<div class="section">
  ...
  <div class="desc">
    <p><strong>时间利用率：</strong>以一周5天×3时段-1（去掉周二下午）= 14个时段为满载基准计算。大部分教室处于20%–60%的合理利用区间，少量专用教室（体育馆、琴房、实验室）利用率较高甚至超载，这也是部分课程排课失败的根本原因。</p>
